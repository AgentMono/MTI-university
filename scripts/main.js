...
function sv(id, title) {
    $.fancybox(RootURL + 'service/host.aspx?i=' + id + '&t=' + escape(title), {
        width: 600,
        height: 350,
        type: 'iframe',
        margin: 0,
...
function ss(v) {
    if (v != '') location.href = RootURL + 'list.aspx?w=' + escape(v);
    else $.fancybox('يجب تحديد كلمات بحث');
...
function showl() {
    lstart = new Date();
    $.fancybox('جاري التحميل.. انتظر قليلا ....<br /><br /><img alt="جاري التحميل" src="' + RootURL + 'images/la.gif" style="width:208px;height:13px;border:0px" />', {
...
function initfb() {
    ...
    $('.profshow,.fancybox').fancybox({
        autoDimensions: false,
        width: 700,
        height: 400,
        hideOnOverlayClick: hoc,
        modal: mod
    });
    ...
    if (isiPad) {
        ...
        $('[target=_iframe]').addClass('iframe').fancybox({
            width: 880,
            height: 650,
            hideOnOverlayClick: hoc,
            modal: mod
...
function ML(o) {
    $.fancybox(RootURL + 'ml.aspx?e=' + (states[1] == 0 ? '' : escape(document.getElementById('txtML').value)) + '&o=' + o, {
        width: 600,
        height: 350,
        type: 'iframe'
    });
